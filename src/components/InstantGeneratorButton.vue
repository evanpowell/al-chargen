...
export default {
  ...
  methods: {
    calculate: function () {
      ...
      charGen.rollStep2();
      charGen.rollStep3();
      console.log(charGen.character.languages);
      console.log(
        JSON.stringify(
          charGen.character.origins.provincialOrigins.regionalLanguages.map(
            ({ language }) => language
          )
        )
      );
      ...
      // const community = s3.rollCommunity();
      // const culturalValues = s3.rollCulturalValues();
      // const reputation = s3.rollReputation();
      // const connection = s3.rollConnection();
