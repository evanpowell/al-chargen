...
export default class CharacterGenerator extends Step6 {
  ...
  applyModification = (modification) => {
    ...
    switch (type) {
      ...
      case "appearance": {
        const newAppearance = Math.round(
          this.character.appearance.initial[name] * modification.multiplier
        );
  ...
  getAllModifications = () => {
    ...
    this.character.appearance.distinguishingFeatures.forEach(
      ({ modifications }) => {
        if (modifications) {
  ...
  fillDistinguishingFeatures = () => {
    this.character.appearance.distinguishingFeatures =
      this.character.appearance.distinguishingFeatures.map((feature) => {
        return {
          ...feature,
  ...
  mapSkillsToFieldsObject = () => {
    return this.character.skills.reduce(
      (fieldsObj, { name, baseAttribute, points }, i) => {
  ...
  mapProficienciesToFieldsObject = () => {
    return this.character.proficiencies.reduce(
      (fieldsObj, { name, baseAttribute, minRequired, points }, i) => {
  ...
  mapToPdfFields = () => {
    const adjustments = Object.entries(
      this.character.attributes.adjustments
    ).reduce(this.showPlusHideZerosReducer, {});

    const bonusPenalties = Object.entries(
      this.character.attributes.bonusPenalties
    ).reduce(this.showPlusHideZerosReducer, {});
    ...
    return {
      ...
      ACC: [this.character.attributes.initial.acc],
      ...
      "Attribute AdjustmentsPRE": [adjustments.acc],
      ...
      "Adjusted ScorePRE": [this.character.attributes.final.acc],
      ...
      "Bonus PenaltyPRE": [bonusPenalties.acc],
